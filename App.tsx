    ...
    setTimeout(() => {
        ...
        if (savedData) {
            try {
                ...
                // If saved cable types exist, use them, otherwise use common types
            ...
            // STRICT LOGIC: Only default ship gets mock data. Others start EMPTY.
            // Cable Types are considered "Common Master Data" so they persist or reset to default.
            if (id === "S1001_35K_FD") {
                setCables(initialCables);
                setNodes(initialNodes);
                setCableTypes(initialCableTypes);
            } else {
                setCables([]); // Empty Schedule
                setNodes([]); // Empty Nodes
                setCableTypes(initialCableTypes); // Common Data
            }
