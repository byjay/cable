...
const TrayVisualizer: React.FC<TrayVisualizerProps> = ({ systemResult, fillRatioLimit = 40 }) => {
    ...
    return (
        <div className="h-full w-full bg-slate-100 flex flex-col overflow-hidden">
                        ...
                        return (
                            <g key={`tier-${idx}`}>
                                ...
                                <text
                                    ...
                                    fill="#334155"
                                >
                                    T{idx + 1} ({tier.cables.length}개, {tier.fillRatio.toFixed(0)}%)
                                </text>
            ...
            {/* Cable List - Scrollable, responsive */}
            <div className="bg-white border-t border-slate-300 shrink-0 max-h-24 sm:max-h-28 overflow-y-auto">
                <div className="flex flex-wrap gap-1 p-1.5 sm:p-2">
                    {processedTiers.flatMap(tier =>
                        tier.cables.map(c => (
                            <div
                                key={c.id}
                                className="flex items-center gap-1 px-1.5 py-0.5 rounded text-[8px] sm:text-[10px] border border-slate-200"
                                style={{ backgroundColor: getTypeColor(c.type) + '30' }}
                            >
                                <span className="w-3 h-3 sm:w-4 sm:h-4 bg-slate-800 text-white rounded text-[7px] sm:text-[8px] flex items-center justify-center font-bold">
                                    {c.displayIndex}
                                </span>
                                <span className="font-medium text-slate-700 max-w-[60px] sm:max-w-none truncate">{c.name}</span>
                                <span className="text-slate-500">Ø{c.od}</span>
                            </div>
                        ))
                    )}
                </div>
