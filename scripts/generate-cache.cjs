...
class RoutingService {
    ...
    findRoute(start, end, checkNodeStr) {
    ...
    calculatePathWithWaypoints(start, end, waypoints) {
        ...
        for (const waypoint of waypoints) {
            ...
            if (segment.distance < 0)
                return { path: [], distance: -1, error: `Cannot reach waypoint ${waypoint}` };
        ...
        if (finalSegment.distance < 0)
            return { path: [], distance: -1, error: `Cannot reach destination ${end}` };
    ...
    dijkstra(start, end) {
        if (!this.graph[start] || !this.graph[end])
            return { path: [], distance: -1, error: "Node not found" };
        if (start === end)
