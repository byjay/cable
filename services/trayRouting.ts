...
export const buildGraph = (nodes: NodeData[]): Record<string, GraphNode> => {
    ...
    nodes.forEach(node => {
        ...
        // Handle both comma and potential formatting issues
...
export const routeCables = (cables: CableData[], nodes: NodeData[]): CableData[] => {
    ...
    return cables.map(cable => {
        // If path is already calculated (e.g. from Main App), keep it, unless we want to force re-route
        // But this function implies calculating it.
