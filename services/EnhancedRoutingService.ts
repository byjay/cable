...
import { EnhancedLevelMapService } from './EnhancedLevelMapService';
...
export class EnhancedRoutingService {
  private graph: Graph = {};
  private enhancedLevelMapService: EnhancedLevelMapService;

  constructor(nodes: Node[]) {
    this.buildGraph(nodes);
    this.enhancedLevelMapService = new EnhancedLevelMapService(nodes);
  }
  ...
  public findRoute(start: string, end: string, checkNodeStr?: string): RouteResult {
    ...
    const levelRoute = this.enhancedLevelMapService.findRoute(start, end);
  ...
  // Get enhanced level map visualization data
  public getLevelMapData(): any {
    return this.enhancedLevelMapService.getVisualizationData();
  }

  // Get navigation map for debugging
  public getNavigationMap(): any {
    return this.enhancedLevelMapService.getNavigationMap();
  }

  // Get enhanced level info
  public getLevelInfo(level: number): any {
    return this.enhancedLevelMapService.getLevelInfo(level);
  }

  // Get all levels
  public getAllLevels(): any {
    return this.enhancedLevelMapService.getAllLevels();
  }

  // Get inter-level connections
  public getInterLevelConnections(): any {
    return this.enhancedLevelMapService.getInterLevelConnections();
  }
